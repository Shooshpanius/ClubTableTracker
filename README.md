...
**ClubTableTracker** — полнофункциональная система для управления столами и бронированием мест в настольных клубах. Включает **веб-приложение** (ASP.NET Core + React + TypeScript + Vite) и **десктопный клиент** (Avalonia).
...
│
├── ClubTableTracker.avalonia/        # Десктопный клиент Avalonia
│   └── ClubTableTracker.avalonia/
│       ├── ViewModels/
│       │   ├── MainViewModel.cs      # Корневая ViewModel, навигация
│       │   ├── LoginViewModel.cs     # Экран входа (JWT-токен)
│       │   ├── HomeViewModel.cs      # Главный экран с вкладками
│       │   ├── ClubListViewModel.cs  # Список клубов и заявки
│       │   ├── BookingsViewModel.cs  # Бронирования
│       │   └── ProfileViewModel.cs   # Профиль пользователя
│       └── Services/
│           ├── ApiConfig.cs          # URL бэкенда
│           ├── ApiService.cs         # HTTP-клиент
│           └── TokenService.cs       # Хранение JWT-токена
│
...
### Десктопный клиент (Avalonia)
Десктопное приложение для Windows / Linux / macOS. Для входа необходимо вставить JWT-токен, полученный из веб-приложения. Функциональность: просмотр клубов, управление членством, список бронирований, профиль. Приложение работает через тот же API по адресу `https://club.wh40kcards.ru/api`.
