// Календарь ивентов клуба (legacy-дизайн). Цвет — у каждого события свой (детерминированно по id).
...
export default function EventCalendar({ events }: Props) {
  ...
  const renderDayBadges = (day: Date) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginTop: 2 }}>
        ...
        {rest > 0 && (
          <div style={{ fontSize: 9, color: '#aaa', textAlign: 'center', lineHeight: '12px' }}>+{rest}</div>
        )}
      </div>
    )
  }
  ...
  const legendItemStyle: React.CSSProperties = {
    ...
    background: '#0f3460', border: '1px solid #0f3460',
  ...
  return (
    <div style={{ background: '#16213e', border: '1px solid #0f3460', borderRadius: 8, padding: 16, userSelect: 'none' }}>
                ...
                return (
                  <td
                    ...
                    style={{
                      ...
                      outline: isSelected ? '2px solid #e94560' : isToday ? '2px solid #4caf50' : '1px solid #1a2a50',
                      outlineOffset: -2,
                    }}
                  >
                    <div>{day.getDate()}</div>
      ...
      <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ fontSize: 11, color: '#666', marginBottom: 2 }}>Мероприятия месяца:</div>
        {monthEvents.length === 0 && (
          <div style={{ fontSize: 12, color: '#aaa' }}>В этом месяце событий нет</div>
        )}
        {monthEvents.map(ev => (
          <button key={ev.id} style={legendItemStyle} onClick={() => goToEvent(ev)} title="Перейти к дате начала">
            ...
            <span style={{
              color: '#eee', fontSize: 12, minWidth: 0,
              overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
            }}>{isCampaign(ev) ? '⚔️' : '🏆'} {ev.title}</span>
            <span style={{ marginLeft: 'auto', color: '#aaa', fontSize: 11, whiteSpace: 'nowrap', flexShrink: 0 }}>
              {isCampaign(ev)
