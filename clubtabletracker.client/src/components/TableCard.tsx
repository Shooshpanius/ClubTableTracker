...
export default function TableCard({ table, onClick }: Props) {
  return (
    <div onClick={onClick} style={{
      background: 'var(--gd-surface)', border: '1px solid var(--gd-border)', borderRadius: 8,
      padding: 16, cursor: onClick ? 'pointer' : 'default', marginBottom: 8
    }}>
      <h4 style={{ margin: '0 0 4px' }}>Table #{table.number}</h4>
      <div style={{ color: 'var(--gd-fg-muted)', fontSize: 13 }}>Size: {table.size}</div>
      <div style={{ color: 'var(--gd-fg-muted)', fontSize: 13 }}>Games: {table.supportedGames ? table.supportedGames.split('|').filter(Boolean).join(', ') : 'N/A'}</div>
    </div>
  )
}
